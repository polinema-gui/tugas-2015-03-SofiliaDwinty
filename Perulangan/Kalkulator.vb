Public Class Kalkulator
    ...
    Private Sub btn_proses_Click(sender As Object, e As EventArgs) Handles btn_proses.Click
        Try
            Dim barisMatriks1 As Integer = text_matrik1.Lines.Count
            Dim kolomMatriks1 As Integer = text_matrik1.Lines.ElementAt(0).Split(" ").Length
            Dim barisMatriks2 As Integer = text_matrik1.Lines.Count
            Dim kolomMatriks2 As Integer = text_matrik1.Lines.ElementAt(0).Split(" ").Length
    ...
    Private Sub cek(ByVal baris1 As Integer, baris2 As Integer, kolom1 As Integer, kolom2 As Integer)
        ...
        For baris = 0 To baris1 - 2
            If (text_matrik1.Lines.ElementAt(baris).Split(" ").Length <> text_matrik1.Lines.ElementAt(baris + 1).Split(" ").Length) Then
                tmp = False
            End If
        Next
        For baris = 0 To baris2 - 2
            If (text_matrik2.Lines.ElementAt(baris).Split(" ").Length <> text_matrik2.Lines.ElementAt(baris + 1).Split(" ").Length) Then
                tmp = False
            End If
        Next
        If (tmp = False) Then
            Me.cekKolom = False
            Convert.ToInt32(text_matrik1.Text)
            Convert.ToInt32(text_matrik2.Text)
        ...
        If (Me.cekKolom) Then
            For baris As Integer = 0 To baris1 - 1
                For kolom As Integer = 0 To kolom1 - 1
                    nilai1(baris, kolom) = text_matrik1.Lines.ElementAt(baris).Split(" ").ElementAt(kolom)
                    nilai2(baris, kolom) = text_matrik2.Lines.ElementAt(baris).Split(" ").ElementAt(kolom)
                Next
            Next
        End If
    End Sub
